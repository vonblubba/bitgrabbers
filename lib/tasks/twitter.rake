...
namespace :twitter do
  ...
  task :post, [:dry_run] => :environment do |t, args|
		...
		text =  truncate(screenshot.title, length: availalbe_chars, separator: ' ')

		client.update_with_media("#{text} #{url} #{hashtags}", File.open(Rails.root.join(screenshot.image.low_quality.path)))
  end
end
